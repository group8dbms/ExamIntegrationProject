...
async function createZeroMarkOutcome({ client, examId, studentId, submissionId, evaluatorId, integrityScore = 0, feedback, metadata }) {
  const totalMarks = await computeExamTotalMarks(client, examId);
  ...
  await client.query(
    `
      ...
      DO UPDATE SET
        ...
        evaluated_at = NOW()
    `,
    [submissionId, evaluatorId, feedback, JSON.stringify(metadata)]
  );
  ...
  await client.query(
    `
      ...
      VALUES ($1, $2, $3, $4, 0, $5, NULL, FALSE, 'draft')
      ON CONFLICT (submission_id)
      DO UPDATE SET
        ...
        status = 'draft'
    `,
    [examId, studentId, submissionId, totalMarks, integrityScore]
  );
}
...
async function closeAttempt({ client, examId, studentId, attemptNo, currentAnswers, actorUserId, ipAddress, reason }) {
  ...
  const savedSubmission = await client.query(
    `
      ...
      VALUES ($1, $2, $3, $4::jsonb, $4::jsonb, 1, NOW(), 'submitted', $5::jsonb)
      ON CONFLICT (exam_id, student_id, attempt_no)
      DO UPDATE SET
        current_answers = EXCLUDED.current_answers,
        final_answers = COALESCE(answer_submission.final_answers, EXCLUDED.final_answers),
    ...
    [
      ...
      JSON.stringify({
        systemGenerated: "closed_attempt",
        autoReason: reason
      })
    ]
  );
  ...
  await createZeroMarkOutcome({
    client,
    examId,
    studentId,
    submissionId: savedSubmission.rows[0].id,
    evaluatorId: attempt.created_by,
    integrityScore: attempt.suspicion_score,
    feedback: "Auto-evaluated as 0 marks because the student closed the exam attempt.",
    metadata: {
      autoGenerated: "closed_attempt",
      rawScore: 0,
      penalty: 0,
      reason
    }
