...
export default function EvaluatorPage({ session, onLogout, setMessage }) {
  ...
  const evaluationLocked = useMemo(
    () => Boolean(selectedSubmission && !selectedSubmission.autoEvaluated && selectedSubmission.evaluatedAt),
    [selectedSubmission]
  );
  ...
  async function saveEvaluation(event) {
    ...
    if (selectedSubmission.autoEvaluated) {
      setMessage("Auto-evaluated attempts are frozen and cannot be changed.");
      return;
    }

    if (evaluationLocked) {
      setMessage("Marks for this submission have already been saved and are now locked.");
      return;
    }
            ...
            {submissions.map((item) => <button key={item.submissionId} type="button" className={selectedSubmissionId === item.submissionId ? "publish-card ready" : "publish-card muted"} onClick={() => setSelectedSubmissionId(item.submissionId)}>
              ...
              {item.autoEvaluated ? (
                <p className="info-line">
                  {item.autoGeneratedReason === "closed_attempt"
                    ? "Auto-evaluated with 0 marks because the attempt was closed."
                    : "Auto-evaluated with 0 marks because the student did not appear before the deadline."}
                </p>
              ) : null}
            </button>)}
          ...
          {selectedSubmission ? <>
            ...
            <form className="single-column" onSubmit={saveEvaluation}>
              <div className="question-preview-card recheck-summary-card">
                ...
                <button type="button" className="secondary-button" disabled={selectedSubmission.autoEvaluated || evaluationLocked} onClick={() => setMarkForm((current) => ({ ...current, awardedMarks: selectedSubmission.autoAwardedMarks, overrideComment: "" }))}>
                  Use Auto Total
                </button>
              </div>
              {selectedSubmission.autoEvaluated ? (
                <p className="info-line">
                  This marks entry is frozen because the script was auto-evaluated as
                  {selectedSubmission.autoGeneratedReason === "closed_attempt" ? " a closed attempt" : " not appeared"}.
                </p>
              ) : evaluationLocked ? (
                <p className="info-line">This marks entry is locked because the evaluator has already saved the marks for this submission.</p>
              ) : null}
              <label className="field"><span>Awarded Marks</span><input type="number" min="0" max={selectedSubmission.totalMarks} step="0.5" value={markForm.awardedMarks} onChange={(event) => setMarkForm({ ...markForm, awardedMarks: event.target.value })} required disabled={selectedSubmission.autoEvaluated || evaluationLocked} /></label>
              {!selectedSubmission.autoEvaluated && !evaluationLocked && Number(markForm.awardedMarks) !== Number(selectedSubmission.autoAwardedMarks) ? (
              ...
              <label className="field"><span>Evaluator Feedback</span><textarea rows="5" value={markForm.feedback} onChange={(event) => setMarkForm({ ...markForm, feedback: event.target.value })} placeholder="Add evaluator remarks" disabled={selectedSubmission.autoEvaluated || evaluationLocked} /></label>
              <div className="form-actions"><button className="primary-button" type="submit" disabled={selectedSubmission.autoEvaluated || evaluationLocked}>Save Marks</button></div>
            </form>
