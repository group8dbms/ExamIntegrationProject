          ...
          {selectedSubmission ? <>
            ...
            <form className="single-column" onSubmit={saveEvaluation}>
              <div className="question-preview-card recheck-summary-card">
                ...
                <button type="button" className="secondary-button" disabled={selectedSubmission.autoEvaluated} onClick={() => setMarkForm((current) => ({ ...current, awardedMarks: selectedSubmission.autoAwardedMarks, overrideComment: "" }))}>
                  Use Auto Total
                </button>
              </div>
              ...
              <label className="field"><span>Awarded Marks</span><input type="number" min="0" max={selectedSubmission.totalMarks} step="0.5" value={markForm.awardedMarks} onChange={(event) => setMarkForm({ ...markForm, awardedMarks: event.target.value })} required disabled={selectedSubmission.autoEvaluated} /></label>
              {!selectedSubmission.autoEvaluated && Number(markForm.awardedMarks) !== Number(selectedSubmission.autoAwardedMarks) ? (
              ...
              <label className="field"><span>Evaluator Feedback</span><textarea rows="5" value={markForm.feedback} onChange={(event) => setMarkForm({ ...markForm, feedback: event.target.value })} placeholder="Add evaluator remarks" disabled={selectedSubmission.autoEvaluated} /></label>
              <div className="form-actions"><button className="primary-button" type="submit" disabled={selectedSubmission.autoEvaluated}>Save Marks</button></div>
            </form>
